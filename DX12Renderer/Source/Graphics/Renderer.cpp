...
void Renderer::Render()
{
    ...
    */
    ...
    commandList2->SetScissorRects(1, &s_Instance->m_ScissorRect);
    commandList2->SetRenderTargets(1, &tmRtv, &tmDsv);

    // Set pipeline state and root signature
    commandList2->SetPipelineState(s_Instance->m_RenderPasses[RenderPassType::TONE_MAPPING]->GetPipelineState());

    // Set tone mapping vertex buffer
    commandList2->SetVertexBuffers(0, 1, *s_Instance->m_ToneMapVertexBuffer.get());
    commandList2->SetIndexBuffer(*s_Instance->m_ToneMapIndexBuffer.get());
    ...
    commandList2->DrawIndexed(s_Instance->m_ToneMapIndexBuffer->GetBufferDesc().NumElements, 1);
